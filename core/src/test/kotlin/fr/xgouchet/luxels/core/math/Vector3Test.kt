...
import fr.xgouchet.luxels.core.math.Vector3.Companion.UNIT
import fr.xgouchet.luxels.core.math.Vector3.Companion.X_AXIS
import fr.xgouchet.luxels.core.math.Vector3.Companion.Y_AXIS
import fr.xgouchet.luxels.core.math.Vector3.Companion.Z_AXIS
import fr.xgouchet.luxels.core.math.Vector3.Companion.fromComponents
import fr.xgouchet.luxels.core.math.Vector3.Companion.fromSpherical
...
class Vector3Test : DescribeSpec({
    ...
    describe("dot product") {
        ...
        it("is distributive over addition: u · (v + w) = (u · v) + (u · w)") {
            checkAll(vector3Arb(), vector3Arb(), vector3Arb()) { u, v, w ->
                val `u · (v + w)` = u dot (v + w)
                val `(u · v) + (u · w)` = (u dot v) + (u dot w)

                `u · (v + w)` shouldBeCloseTo `(u · v) + (u · w)`
            }
        }

        it("is bilinear: u · (xv + w) = x(u · v) + (u · w)") {
            checkAll(vector3Arb(), vector3Arb(), vector3Arb(), doubleArb()) { u, v, w, x ->
                val `u · (xv + w)` = u dot ((v * x) + w)
                val `x(u · v) + (u · w)` = ((u dot v) * x) + (u dot w)

                `u · (xv + w)` shouldBeCloseTo `x(u · v) + (u · w)`
            }
        }
    ...
    describe("cross product") {
        ...
        it("is distributive over addition: u ⨯ (v + w) = (u ⨯ v) + (u ⨯ w)") {
            checkAll(vector3Arb(), vector3Arb(), vector3Arb()) { u, v, w ->
                val `u ⨯ (v + w)` = u cross (v + w)
                val `(u ⨯ v) + (u ⨯ w)` = (u cross v) + (u cross w)
        ...
        it("is consistent with scalar multiplication: ku ⨯ v = u ⨯ kv = k(u ⨯ v)") {
            checkAll(vector3Arb(), vector3Arb(), doubleArb()) { u, v, k ->
                ...
                val `k(u ⨯ v)` = (u cross v) * k

                `ku ⨯ v` shouldBeCloseTo `u ⨯ kv`
                `ku ⨯ v` shouldBeCloseTo `k(u ⨯ v)`
            }
        }

        it("satisfies the Jacobi identity: [u ⨯ (v ⨯ w)] + [v ⨯ (w ⨯ u)] + [w ⨯ (u ⨯ v)] = null") {
            checkAll(vector3SmallArb(), vector3SmallArb(), vector3SmallArb()) { u, v, w ->
                val `u ⨯ (v ⨯ w)` = u cross (v cross w)
                val `v ⨯ (w ⨯ u)` = v cross (w cross u)
                val `w ⨯ (u ⨯ v)` = w cross (u cross v)

                val sum = `u ⨯ (v ⨯ w)` + `v ⨯ (w ⨯ u)` + `w ⨯ (u ⨯ v)`
        ...
        it("allows triple product: u · (v ⨯ w) = v · (w ⨯ a) = w · (u ⨯ v)") {
            checkAll(vector3Arb(), vector3Arb(), vector3Arb()) { u, v, w ->
                val `u · (v ⨯ w)` = u dot (v cross w)
                val `v · (w ⨯ u)` = v dot (w cross u)
                val `w · (u ⨯ v)` = w dot (u cross v)

                `u · (v ⨯ w)` shouldBeCloseTo `v · (w ⨯ u)`
                `u · (v ⨯ w)` shouldBeCloseTo `w · (u ⨯ v)`
            }
        }
        ...
        it("is consistent with dot product and squaredLength: ‖u ⨯ v‖² = ‖u‖² ‖v‖² - (u · v)²") {
            checkAll(vector3Arb(), vector3Arb()) { u, v ->
                ...
                val `‖u‖² ‖v‖² - (u · v)²` = (u.squaredLength() * v.squaredLength()) - ((u dot v) * (u dot v))

                `‖u ⨯ v‖²` shouldBeCloseTo `‖u‖² ‖v‖² - (u · v)²`
            }
        }
